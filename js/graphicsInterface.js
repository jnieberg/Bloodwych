		...
		if (typeof uiJson[item].gfx.recolour !== 'undefined') {
			...
			if (typeof uiJson[item].gfx.classColours !== 'undefined') {
				...
				if (uiJson[item].gfx.multiple.direction === 'x') {
					for (let x = 0; x < uiJson[item].gfx.multiple.size; x++) {
					...
					for (let y = 0; y < uiJson[item].gfx.multiple.size; y++) {
...
function drawSpellBook(p, ui, dr) {
	...
	if (ch.selectedSpell === null && !start) {
		...
		var t = recolourUiGfx(font[94], colourData.GREEN, colourData.RED);
		...
		if (!p.showSpellText) {
			...
			t = showStatusBar(cst, 1.0, 62, colourData.RED);
		...
		if (ch !== null) {
			if (c === 0) {
				if (p.uiLeftPanel.champs[c].opened) {
					...
					t = drawCharacter(monster[TOWER_CHAMPIONS][cid], 0, 0, p, {
						x: 0,
						y: -1
					}, true, false);
...
function leftUIStats(p) {
	...
	if (p.uiLeftPanel.champs[0].opened) {
		...
		for (var c = 0; c < p.champion.length; c++) {
			var c1 = ch[c];
			champ = p.getChampion(c1);
			if (champ !== null) {
				rgb = getClassColour(champ.colour);
				hp = Math.floor(21 * champ.stat.hp / champ.stat.hpMax);
				if (hp < 0) {
					hp = 0;
				}
		...
		champ = p.getChampion(p.championLeader);
		if (champ !== null) {
			hp = Math.floor(35 * champ.stat.hp / champ.stat.hpMax);
...
function colourSpellPage(dr, ch, img) {
	...
	if (dr) {
		page = (ch.spellBookPage + 1) % SPELL_PAGE_MAX;
	} else {
		page = ch.spellBookPage;
	}
